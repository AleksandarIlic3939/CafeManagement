...
public class UserServiceImpl implements UserService {
            ...
            if (auth.isAuthenticated()) {
                if (customerUsersDetailsService.getUserDetails().getStatus().equalsIgnoreCase("true")) {
                    return new ResponseEntity<String>("{\"token\":\"" + jwtUtil.generateToken(
                            customerUsersDetailsService.getUserDetails().getEmail(),
        ...
        try {
            if (jwtFilter.isAdmin()) {
                ...
                if (!optional.isEmpty()) {
                    ...
                    sendMailToAllAdmins(requestMap.get("status"), optional.get().getEmail(), userDao.getAllAdminsEmail());
    ...
    public ResponseEntity<String> forgotPassword(Map<String, String> requestMap) {
        try {
            ...
            if (!Objects.isNull(user) && !Strings.isNullOrEmpty(user.getEmail())) {
                emailUtils.forgotPasswordMessage(user.getEmail(), "Credentials by Cafe Management System", user.getPassword());
    ...
    private void sendMailToAllAdmins(String status, String user, List<String> allAdminsEmail) {
        ...
        if (status != null && status.equalsIgnoreCase("true")) {
            emailUtils.sendSimpleMessage(admin, "Account approved", "USER:- " + user + "\n is approved by \nADMIN:- " + admin, allAdminsEmail);
        } else {
            emailUtils.sendSimpleMessage(admin, "Account disabled", "USER:- " + user + "\n is disabled by \nADMIN:- " + admin, allAdminsEmail);
        }
    }
